...
public class Teacher extends Person {
        ...
        for (int i = 0; i < teachers.size(); ++i) {
            ...
            for (int j = 0; j < teachers.get(i).getConsumptions().size(); ++j) {
                if (j == 0) {
                    System.out.println("用电量：" + teachers.get(i).getConsumptions().get(j).getConsumption() + " 度");
                    System.out.printf("应交电费：%.2f 元\n",teachers.get(i).getConsumptions().get(j).getConsumption()*0.588);
                    System.out.println("是否缴电费：" + (teachers.get(i).getConsumptions().get(j).isPay() == true ? "已缴" : "未缴"));
                }
                if (j == 1) {
                    System.out.println("用水量：" + teachers.get(i).getConsumptions().get(j).getConsumption() + " 千升");
                    System.out.printf("应交水费：%.2f 元\n",teachers.get(i).getConsumptions().get(j).getConsumption()*3.10);
                    System.out.println("是否缴水费：" + (teachers.get(i).getConsumptions().get(j).isPay() == true ? "已缴" : "未缴"));
                }
            }
        }
    }
    ...
    //统计教工使用水电情况
    public static void statistics(List<Person> teachers){
        if (teachers.size() == 0) {
            System.out.println("-- 暂无教工信息，请先手动添加 --");
        }

        List<Person> teachers1 = teachers ;

        //从大到小排序
        for(int i = 0;i < teachers1.size() - 1;++ i){
            for(int j = 0;j < teachers1.size() - 1 - i;++ j){
                if(teachers1.get(j).getConsumptions().get(0).getConsumption() < teachers1.get(j+1).getConsumptions().get(0).getConsumption()){
                    Person temp = teachers1.get(j) ;
                    teachers1.set(j+1,teachers1.get(j)) ;
                    teachers1.set(j,temp) ;
                }
            }
        }
        ...
        for(int i = 0;i < teachers1.size();++ i){
            ...
            System.out.print(teachers1.get(i).getName() + "\t");
            System.out.printf("%.2f\t\t",teachers1.get(i).getConsumptions().get(0).getConsumption());
            System.out.printf("%.2f\t\t",teachers1.get(i).getConsumptions().get(1).getConsumption());
            System.out.printf("%.2f\t",teachers1.get(i).getConsumptions().get(0).getConsumption()*0.588);
            System.out.printf("%.2f\t",teachers1.get(i).getConsumptions().get(1).getConsumption()*3.10);
            System.out.print(teachers1.get(i).getConsumptions().get(0).isPay() + "\t\t");
            System.out.println(teachers1.get(i).getConsumptions().get(1).isPay());
        }
    }
